class Pydelphin < Formula
  ...
  depends_on "python"
  depends_on "dantiston/delphin/ace" # => :recommended
  ...
  test do
    ...
    # Make the profile
    system bin/"delphin", "mkprof", testpath/"tests", "--delimiter=\"@\"", "--relations", "relations.txt", "--input", "sentences.txt"
    assert_predicate testpath/"tests"/"item", :exist?

    # Compile the grammar & process profile
    system Formula["dantiston/delphin/ace"].bin/"ace", "-g", "config.tdl", "-G", testpath/"grammar.dat"
